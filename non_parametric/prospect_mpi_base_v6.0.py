...
def run_prospector(this_obj, **extras):
    ...
    # Build the model
    model, model_params = build_model(obs, fixed_metallicity=0.0, add_duste=ADD_DUSTE,
                add_neb=False, add_agn=ADD_AGN, change_dirich_prior=False)

    NDIM = model.ndim

    model_file = '{0}_f0_model_desc.txt'.format(src_tag)
    ...
    from prospect.io import write_results as writer
    hfile = "{0}/EN1_{1}_dynesty_output.h5".format(obj_dir,obs['radio_id'])
    ...
    zind = np.where(np.array(model.free_params) == 'z_fraction')[0][0]
    theta_labels = list(np.concatenate([model.free_params[:zind],
                    np.array(['z_fraction_'+str(ii+1) for ii in range(model_params['z_fraction']['N'])]),
                    model.free_params[zind+1:]]))
    ...
    ldust_arr, ldust2_arr, mfrac_arr, mcurr_arr = [], [], [], []

    # Define arrays for the sfh plots
    # Getting the sfr
    randplts = 5
    shouldiplot = np.random.choice(range(len(flatchain)), randplts, p=weights)
    sfh_randplot = []
    tcolors = ['tomato','lightgreen', 'violet', 'silver','sandybrown', 'green',  'aqua', 'deeppink', 
               'fuchsia', 'darkorchid']
               
    agebins = model.params['agebins']
    nagebins = np.shape(model.params['agebins'])[0]

    all_sfr_arr, sfrqtiles, mstar100_arr, sfr100_arr, ssfr100_arr = [],[],[],[],[]

    for xx in range(nagebins):
        all_sfr_arr.append([])
        sfrqtiles.append([])
    ...
    for tt in range(len(flatchain)):
        ...
        this_mcurr = this_state[theta_keys['total_mass']]*this_mfrac
        ldust = get_ldust(spec_wave,magp_phot_Llambda(spec_wave, pros_to_magp_phot(spec_wave, this_spec, obs['z_best'])))

        ldust2 = get_ldust(spec_wave*(1+obs['z_best']),magp_phot_Llambda(spec_wave, pros_to_magp_phot(spec_wave, this_spec, obs['z_best'])))
        ldust_arr.append(ldust)
        ldust2_arr.append(ldust2)
        mfrac_arr.append(this_mfrac)
        mcurr_arr.append(this_mcurr)

        # SFR Parts
        
        this_totmass = this_state[nagebins]
        z_frac = this_state[1:nagebins]
        
        this_mass_arr = zfrac_to_masses(total_mass=this_totmass, z_fraction=z_frac, agebins=agebins)
        this_sfr_arr = zfrac_to_sfr(total_mass=this_totmass, z_fraction=z_frac, agebins=agebins)
        
        for tm in range(len(this_sfr_arr)): all_sfr_arr[tm].append(this_sfr_arr[tm])  
        if tt in shouldiplot: sfh_randplot.append(this_sfr_arr)
            
        mstar100_arr.append(this_mass_arr[0]+this_mass_arr[1])
            
        sfrnum = (10**agebins[0][1] - 10**agebins[0][0])*this_sfr_arr[0] + (10**agebins[1][1] - 10**agebins[1][0])*this_sfr_arr[1]
        sfrden = (10**agebins[1][1] - 10**agebins[0][0])
        this_sfr = sfrnum/sfrden
        sfr100_arr.append(this_sfr)
        ssfr100_arr.append(this_sfr/this_totmass)
    ...
    for tm in range(len(this_sfr_arr)):
        sfrqtiles[tm] = weighted_quantile(all_sfr_arr[tm],[0.16,0.5,0.84],sample_weight=weights)
    ...
    best_mcurr = theta_max[theta_keys['total_mass']]*best_mfrac


    best_totmass = theta_max[nagebins]
    best_z_frac = theta_max[1:nagebins]

    best_mass_arr = zfrac_to_masses(total_mass=best_totmass, z_fraction=best_z_frac, agebins=agebins)
    best_sfr_arr = zfrac_to_sfr(total_mass=best_totmass, z_fraction=best_z_frac, agebins=agebins)

    mstar100_best = best_mass_arr[0]+best_mass_arr[1]

    sfrnum = (10**agebins[0][1] - 10**agebins[0][0])*best_sfr_arr[0] + (10**agebins[1][1] - 10**agebins[1][0])*best_sfr_arr[1]
    sfrden = (10**agebins[1][1] - 10**agebins[0][0])
    best_sfr_100 = sfrnum/sfrden
    best_ssfr_100 = best_sfr_100/best_totmass
    ...
    p16_ldust2, p50_ldust2, p84_ldust2 = weighted_quantile(ldust2_arr,[0.16,0.5,0.84],sample_weight=weights)
    ...
    with open(sfhfile,'w') as f: # Save SFH to a file
        print('# agebin start [yr]    agebin end [yr]    sfh_16ptile    sfh_50ptile    sfh_84ptile [Msol/yr]', file=f)
        for ii in range(nagebins):
            tx = np.linspace(10**agebins[ii][0],10**agebins[ii][1],10)
            plt.hlines(y=sfrqtiles[ii][1], xmin=10**agebins[ii][0], xmax=10**agebins[ii][1],
                       ls='-.',color='dodgerblue', zorder=1, lw=3)
            plt.fill_between(tx, sfrqtiles[ii][0],sfrqtiles[ii][2], 
                   color='lightblue', alpha=0.2, zorder=1)
            tt = 0
            for tsfhplot in sfh_randplot:
                plt.hlines(y=tsfhplot[ii], xmin=10**agebins[ii][0], xmax=10**agebins[ii][1], 
                           color=tcolors[tt%10], zorder=1, lw=1)
                tt+=1

            print('{0:.5e} {1:.5e} {2:.5e} {3:.5e} {4:.5e}'.format(10**agebins[ii][0], 10**agebins[ii][1], sfrqtiles[ii][0], sfrqtiles[ii][1], sfrqtiles[ii][2]), file=f)

    plt.plot(0,0,ls='-.',color='dodgerblue', zorder=1, lw=3,label='50 %tile')
            
          
        
    plt.xscale('log')
    plt.yscale('log')
    plottitle = plt_tag+ ' - ' + 'SFH.'

    plt.xlim([1e6,obs['age_univ']*1.e9])
    ...
    with open(spitoutfile,'w') as f:
        ...
        print('mstar100_best = ', mstar100_best ,file=f)
        ...
        print('sfr100_best = ', best_sfr_100 ,file=f)
        ...
        print('ssfr100_best = ', best_ssfr_100 ,file=f)
        ...
        # model.fixed_params
        some_fixed_params = ['mass','logzsol','sfh','imf_type','dust_type','dust1']
        for mm in some_fixed_params:
